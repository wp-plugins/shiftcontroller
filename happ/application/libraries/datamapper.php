...
class DataMapper implements IteratorAggregate {
	...
	public static function autoload($class)
	{
		...
		// get the CI instance
		is_null($CI) AND $CI =& ci_get_instance();
	...
	protected static function _load_extensions(&$extensions, $names)
	{
		...
		foreach($names as $name => $options)
		{
			...
			if ( ! file_exists($file))
			{
				...
				{
					$file = NTS_SYSTEM_APPPATH . DataMapper::$config['extensions_path'] . '/' . $name . EXT;
					$ext = $name;
				}
				else
				{
					$file = NTS_SYSTEM_APPPATH . $name . EXT;
